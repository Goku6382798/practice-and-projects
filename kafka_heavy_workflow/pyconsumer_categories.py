...
consumed_logs = []
BATCH_SIZE = 1000
LOG_DIR = '/home/gautam/practice/kafka_heavy_workflow/categories_logs_batch'
FILE_PREFIX = "categories_logs_batch"
...
def save_logs():
    ...
    logging.info(f"Saved {len(consumed_logs)} logs to {log_file_path}")
    consumed_logs.clear()
...
def make_consumer():
    ...
    avro_deserializer = AvroDeserializer(
        ...
        schema_str=CATEGORIES_SCHEMA,
        from_dict=lambda data, ctx: categories(**data)
    )
    string_deseializer = StringDeserializer('utf-8')

    return DeserializingConsumer({
        "bootstrap.servers": os.getenv("BOOTSTRAP_SERVERS"),
        "group.id": os.getenv("CONSUMER_GROUP"),
        "key.deserializer": string_deseializer,
...
def consume_messages():
    ...
    consumer.subscribe(["ecommerce.categories-value"])
    try:
        while True:
