...
songs_df = spark.read.schema(song_schema).csv("s3://gd-aws-de-labs/songs.csv")
...
raw_stream_df = glueContext.create_data_frame.from_options(
    ...
    connection_options={
        "streamARN": "arn:aws:kinesis:us-east-1:418272783111:stream/user_song_streams1",
        "classification": "json",
...
query = metrics_df.writeStream \
    .outputMode("append") \
    .format("parquet") \
    .option("path", "s3://gd-aws-de-labs/music_metrics/") \
    .option("checkpointLocation", "s3://gd-aws-de-labs/checkpoints/music_metrics/") \
    .start()
