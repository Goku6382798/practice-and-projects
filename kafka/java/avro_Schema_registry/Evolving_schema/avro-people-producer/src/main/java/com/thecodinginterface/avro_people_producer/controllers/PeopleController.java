...
public class PeopleController {
    ...
    public List<PersonDto> create(@RequestBody CreatePeopleCommand cmd) {
        ...
        for (int i = 0; i < cmd.getCount(); i++) {
            ...
            people.add(new PersonDto(firstName, lastName, title));

            CompletableFuture<SendResult<String, Person>> future = kafkaTemplate.send(
                    personAvroTopic,
                    (firstName != null ? firstName.toLowerCase() : "unknown"),
                    person
            );
    ...
    private String generateRandomName() {
        String[] firstNames = {"Gautam", "Tony", "Bruce", "Natasha", "Peter", "Clark"};
        String[] lastNames = {"Dixit", "Stark", "Wayne", "Romanoff", "Parker", "Kent"};
