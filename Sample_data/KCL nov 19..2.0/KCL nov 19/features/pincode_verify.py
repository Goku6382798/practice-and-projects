...
def pincode_verify():
    try:
        ...
        if not pincode:
            return jsonify({"statusCode": 400, "message": "Pincode is required", "response": {}})
        if not re.fullmatch(r'^[1-9][0-9]{5}$', pincode):
            return jsonify({"statusCode": 400, "message": "Pincode must be 6 digits and cannot start with 0", "response": {}})
