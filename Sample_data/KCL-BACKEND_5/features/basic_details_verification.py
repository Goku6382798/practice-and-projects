...
def participant_basic_details():
    try:
        ...
        if not contest_id:
            return jsonify({"statusCode": 400, "message": "contest_id is required.", "response": {}})

        # Fetch Eligibility Mapping
        mappings = list(db.eligibility_mapping.find({"contest_id": contest_id, "is_active": True}))
        if not mappings:
            return jsonify({"statusCode": 404, "message": "No active eligibility mapping found", "response": {}})
        ...
        if not eligibilities:
            return jsonify({"statusCode": 404, "message": "No eligibility rules found", "response": {}})
        ...
        for rule in eligibilities:
            if rule["eligibility_name"].lower() == "age" and dob_str:
                ...
                if invalid_dob_format:
                    custom_errors.append("Invalid DOB format. Use DD/MM/YYYY.")
                elif age is not None and not (min_age <= age <= max_age):
                    custom_errors.append(f"Your Age does not lie between {min_age}-{max_age} Years")
        ...
        for rule in eligibilities:
            if rule["eligibility_name"].lower() == "gender" and gender:
                ...
                if gender.lower() != expected.lower():
                    custom_errors.append(f"The contest is for '{expected}' Only")

        # PINCODE
        if pincode:
            if not pincode_valid_format:
                custom_errors.append("Invalid Pincode format")
            elif not pincode_exists:
                custom_errors.append("The Pincode does not exist in our system")
            elif not is_rural:
                custom_errors.append("The Pincode does not lie in Rural India")
        ...
        if dob_str and gender and pincode:
            try:
                ...
                print("Insert Error:", str(e))  # do not stop flow
