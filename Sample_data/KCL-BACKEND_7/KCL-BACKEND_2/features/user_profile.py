    ...
    try:
        ...
        if not user_id:
            return jsonify({"statusCode": 400,
                            "message": translate_message("userid is required",languageCode),
                            "response": {}})
        if not _user_exists(user_id):
            return jsonify({"statusCode": 400,
                            "message": translate_message("Invalid userid (not found)",languageCode),
                            "response": {}})
        ...
        if empty_fields:
            return jsonify({
                "statusCode": 400,
                "message": translate_message("empty",languageCode),
        ...
        if profile_data and profile_id is not None:
            ...
            return jsonify({
                "statusCode": 200,
                "message": translate_message("Profile updated successfully",languageCode),
                "response": {
                    "details": _build_profile_details(saved, languageCode),
        ...
        # --- CREATE ---
            ...
            return jsonify({
                "statusCode": 200,
                "message": translate_message("Profile created successfully",languageCode),
                "response": {
                    "details": _build_profile_details(saved, languageCode),
                    "isprofilesetup": info_after["isprofilesetup"],
                    "ismpinsetup": info_after["ismpinsetup"],
